...
function toCompact(inv) {
  ...
  // and an optional 5th element (a short unit string, e.g. "hrs"). Old 3- and
  // 4-element links still decode unchanged.
  c.i = inv.items.map((it) => {
    const t = [it.name, it.qty, it.priceMinor];
    if (it.unit) {
      // Emit a slot for the discount even when there isn't one, so the unit
      // stays in position 5. Explicit null keeps the tuple length honest.
      t.push(it.discount ? [it.discount.kind === 'pct' ? 0 : 1, it.discount.value] : null);
      t.push(it.unit);
    } else if (it.discount) {
      t.push([it.discount.kind === 'pct' ? 0 : 1, it.discount.value]);
    }
    return t;
  });
...
function fromCompact(c) {
  ...
  const items = (Array.isArray(c.i) ? c.i : []).map((it, idx) => {
    // 3 = [name, qty, price]; 4 = + discount tuple; 5 = + unit string.
    if (!Array.isArray(it) || it.length < 3 || it.length > 5) throw new BadPayload(`Bad line item ${idx + 1}`);
    const [name, qty, priceMinor, disc, unitRaw] = it;
    ...
    // Unit is display-only; drop bad values leniently rather than failing the
    // whole invoice, matching the style-lenient policy.
    const unit = typeof unitRaw === 'string' && unitRaw.trim() && unitRaw.length <= 8
      ? unitRaw.trim() : null;
    return { name, qty, priceMinor, discount, unit };
  });
