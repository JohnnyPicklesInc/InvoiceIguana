...
export function renderQuoteInto(root, inv) {
  ...
  for (const it of inv.items) {
    ...
    // Unit sits next to qty ("5 hrs") and, when present, tags the unit price
    // ($150/hr) so the columns still read together. No unit → same as before.
    qtyTd.textContent = it.unit ? `${it.qty} ${it.unit}` : String(it.qty);
    const priceTd = document.createElement('td');
    priceTd.className = 'item-price';
    const priceStr = money(it.priceMinor, inv.currency);
    priceTd.textContent = it.unit ? `${priceStr}/${it.unit}` : priceStr;
